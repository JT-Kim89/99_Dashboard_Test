...
def parse_startup_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Streamlit 실행 뒤쪽에 붙인 사용자 인자를 읽습니다.

    예:
        streamlit run app.py -- --db "C:\\results\\case_001.db"

    Streamlit 자체 인자와 충돌하지 않도록 parse_known_args()를 씁니다.
    """
...
def get_cli_database_path() -> str:
    """C#/터미널에서 --db로 넘긴 DB 경로를 가져옵니다."""

    return clean_path_text(parse_startup_args().db_path)


def get_env_database_path() -> str:
    """환경변수 FPSO_DASHBOARD_DB로 넘긴 DB 경로를 가져옵니다."""

    return clean_path_text(os.environ.get("FPSO_DASHBOARD_DB", ""))


def get_query_database_path() -> str:
    """URL query string의 db 값을 가져옵니다.

    예:
        http://127.0.0.1:8508/?db=C%3A%5Cresults%5Ccase_001.db

    이미 dashboard가 실행 중일 때 C#이 브라우저만 다시 열어도
    다른 DB 파일을 기본 경로로 보여줄 수 있게 하기 위한 장치입니다.
    """
...
def get_default_database_path_text() -> str:
    """사이드바 DB 경로 입력창의 기본값을 결정합니다.

    우선순위:
    1. URL query의 db
    2. CLI 인자 --db
    3. 환경변수 FPSO_DASHBOARD_DB
    4. 샘플 DB 경로
    """
